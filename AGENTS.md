...
> ⚠️ **Constraint — `removed` covers all instances of the config node.** A bracket-less `removed { from = module.NAME.type.name }` forgets **every** dynamic instance of that resource across **every** instance of the module (all `count`/`for_each` module instances × all resource keys), because OpenTofu cannot scope a `removed` block to a single module instance. This is exactly correct for the common `count = 1` conditional-module pattern (only `[0]` exists). But if the module has **multiple** instances (`count = 2`, or a `for_each` module) and you move only `[0]`, the source `removed` block would also forget the same resource from `[1]`, `[2]`, … even though the `import` side only re-creates `[0]` in the destination — orphaning the other instances' state. **statebridge enforces this:** a cross-layer indexed-module move whose source layer's state contains more than one instance of the module fails at `generate` time with an error listing the offending instances. Only use an indexed-module cross-layer move when the module has a single instance (or move the resource out of every module instance).
>
...
# {"version":2,"source_layer":{"storage_account_name":"myacct","container_name":"tfstate","key":"layers/compute.tfstate"},"conditions":{"resources_exist":[{"layer":".","addresses":["azurerm_vm.web"]}]},"resources":["azurerm_vm.web","azurerm_vnet.main"]}
...
**Fields:**
- `version` — metadata schema version. Written as `2` for newly generated files; absent (treated as `1`) for legacy files generated before versioning. Consumers ignore unknown fields, so older statebridge versions can still read newer files and vice versa.
- `source_layer` — the owning layer's Azure backend coordinates (`storage_account_name`, `container_name`, `key`), discovered from the layer's backend block at generate time. Used to scope blob operations when multiple layers share a single storage container (see below). Omitted for legacy files or when the coordinates could not be discovered.
...
Migration blobs are stored under a flat `migrations/` prefix in the layer's backend container. When **each layer has its own container**, this is unambiguous. When **multiple layers share one container** (a shared storage account + container, distinguished only by their backend `key`), all layers' migration blobs sit next to each other. To prevent one layer from acting on another layer's blobs, statebridge stamps each file with its `source_layer` coordinates and scopes every blob-listing operation:

- **`download`** (per layer): skips blobs whose `source_layer` provably belongs to a different layer (backend account/container/key mismatch). It still downloads legacy blobs (no `source_layer`) and, when the current layer's `key` cannot be determined, falls back to condition-based applicability with a warning.
- **`upload` guard** and **old-version cleanup**: never block on, or delete, a blob owned by a different layer that happens to share the same YAML stem.
- **`prune`** and **auto-prune on `--upload`**: never evaluate/delete another layer's blobs. `prune --force` remains layer-scoped (it will not cross-delete).

This is fully backward compatible: files without `source_layer` behave exactly as before. The `key` is compared as an opaque discriminator — since the container already scopes the blob listing, `key` is the effective distinguisher between layers in a shared container.
